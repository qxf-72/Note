...
> 
> 主频：机器内部主时钟频率，即，时钟周期的倒数。每秒 时钟周期的数量。
...
> 在描述**存储容量、文件大小**时，K、M、G、T 通常用 **2 的幂次**表示。
> 
> 在描述**速率、频率**时，通常用 **10 的幂次**表示。

采用吉普森法，综合考虑每条指令操作时间及在全部操作的占比，现在普遍采用MIPS（百万条指令每秒）、CPI（执行一条指令所需时钟周期）、FLOPS（浮点数运算次数每秒）。
...
<center>
<img src="https://picture-in-md.oss-cn-guangzhou.aliyuncs.com/2023-04-19_093538.png" width="500" />
</center>
