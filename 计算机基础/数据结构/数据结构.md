<div align="center"> <img src="https://cdn.nlark.com/yuque/0/2023/jpeg/29674612/1681562060742-9d87be44-5901-4d50-a90f-788d38fa5b12.jpeg" width="500" /> </div>
...
<div align="center"> <img src="https://cdn.nlark.com/yuque/0/2022/png/29674612/1658368160627-3e331883-6807-442c-9fef-9c44386f4baf.png" width="232" /> </div>
...
## 数据结构
数据元素不是孤立存在的，他们之间存在某种关系，数据元素相互之间的关系成为结构，是指相互之间存在一种或多种特定关系的数据元素的集合，数据结构是带结构的数据元素的集合
**三方面的内容**
...
1. 逻辑结构：数据元素之间的逻辑关系
   ...
   2. 与数据的存储结构无关，独立于计算机
   3. 是从具体问题抽象出来的数学模型
2. 物理结构（存储结构）：数据元素及其关系在计算机内存的表示（映像）
   1. 数据元素及其关系在计算机中的存储方式
   2. 是数据结构在计算机中的表示
3. 运算与实现（算法）：对数据元素可以施加的操作以及这些操作在相应存储结构的实现
...
# 查找
## 基本概念
查找表：由同一类型的数据元素构成的集合，由于集合中的数据元素之间存在完全松散的关系，因此查找表是一种非常灵便的数据结构
查找：根据给定的某个值，在查找表中确定一个其关键字等于给定值的数据元素
关键字：数据元素中某项数据项的值，用它来标识一个数据元素

- 主关键字：可以唯一识别一个记录的关键字
- 次关键字：反之，用以识别若干记录的关键字是次关键字

静态查找表：仅作“查询”操作的查找表
动态查找表：将查询结果为不在查找表的数据元素插入导查找表中，或者从查找表中删除其查找结果为在查找表的数据元素
...
把待查关键字key存入表头，起监视哨的作用，可免去每一步都要检测是否查找完毕（原来每一步要检测两次），加快速度，这个改进能使顺序查找在$ST.length>=1000$时，进行一次查找所需平均时间几乎减少一半。
**性能分析**

- 等查找概率下，顺序查找成功的平均查找长度$ASL_{ss}=\frac{n+1}{2}$。
...
### 折半查找
**应用对象**——有序表
先确定待查记录所在范围，然后逐步缩小范围，直到找到或找不到记录为止
**性能分析**

- 折半查找的过程实质是，在一颗判定树上从根节点到目标结点或空结点的过程，判定树虽然不是完全二叉树，但是它的叶子结点所在层次之差最大为1，故具有$n$个结点的判定树的深度为$\lfloor \log _2n \rfloor +1$，故进行比较的关键字的个数至多为$\lfloor \log _2n \rfloor +1$
- 查找概率相等时，查找成功的$ASL_{bs}=\frac{n+1}{n}\log _2\left( n+1 \right) -1$,当n取值较大的时候，$ASL_{bs}=\log _2\left( n+1 \right) -1$
...
- $ASL_{bs}=L_b+L_w$
- 将长度为n的表，均匀分成b块，每块含s个记录
- 若使用顺序查找找所在的块，$ASL_{bs}=\frac{1}{b}\sum_{j=1}^b{j}+\frac{1}{s}\sum_{i=1}^s{i=\frac{b+1}{2}+\frac{s+1}{2}=\frac{1}{2}\left( \frac{n}{s}+s \right) +1}$
- 若使用折半查找找所在的块，$ASL_{bs}\approx \log _2\left( \frac{n}{s} \right) +\frac{s}{2}$
## 树表的查找
当表插入、删除操作频繁时，为维护表的有序性，需要移动表中很多记录。可以改用动态查找表——树表
### 二叉排序树
> 二叉搜索树（BST，Binary Search Tree），也称[二叉排序树](https://so.csdn.net/so/search?q=%E4%BA%8C%E5%8F%89%E6%8E%92%E5%BA%8F%E6%A0%91&spm=1001.2101.3001.7020)或二叉查找树，可以为空，如果不为空，满足以下性质：
> - 非空左子树的所有键值小于其根结点的键值
> - 非空右子树的所有键值大于其根结点的键值
> - 左、右子树都是二叉搜索树
...
- 思想类似于二分查找，可以用递归实现
- 含有n个结点的二叉树的ASL与树的形态有关
   - 最好情况下，二叉排序树是一颗平衡二叉树
   - 最坏情况下，二叉树退化为单支树，查找效率和顺序查找相同

**二叉排序树的插入**

- 先查找然后再对应位置插入，插入结点都是叶子结点，不需要移动其他结点
- 一个无序序列可以通过构造二叉排序树而变成一个有序序列，构造树的过程就是对无序序列进行排序的过程
- 不同的插入次序的序列生成不同的形态的二叉排序树

**二叉排序树的删除**

- 需要保证删除结点后所得到的二叉树让然满足二叉排序树的性质
- 根据需删除结点的的情况，分类讨论
   -  被删除结点时叶子结点，直接删除该节点
   - 只有左子树或右子树，用左子树或右子树替换它
   - 既有左子树又有右子树，思路——先找到在有序序列中待删除结点的上一个结点，进行结点替换，然后删除
      - <div align="center"> <img src="https://cdn.nlark.com/yuque/0/2022/png/29674612/1661567557069-9c1e53ef-3d0f-47d1-9a97-cba21d094ee2.png#averageHue=%23fefefe&from=url&height=497&id=QbhQh&originHeight=1079&originWidth=1287&originalType=binary&ratio=1&rotation=0&showTitle=false&size=371608&status=done&style=none&title=&width=593" width="593" /> </div>


      - <div align="center"> <img src="https://cdn.nlark.com/yuque/0/2022/png/29674612/1661567582322-578b36ba-0ef9-453b-80a9-4377bc7aca5f.png#averageHue=%23fefefe&from=url&height=232&id=IbDSD&originHeight=465&originWidth=1190&originalType=binary&ratio=1&rotation=0&showTitle=false&size=130884&status=done&style=none&title=&width=594" width="594" /> </div>


### 平衡二叉树
> 平衡二叉树又称AVL树，平衡二叉树可以是空树，若不为空，则满足以下性质
> - 左子树和右子树的高度之差的绝对值小于等于1
> - 左子树和右子树也是平衡二叉树

为方便描述每个结点的左子树和右子树高度之差，给每个结点附加一个数字，称为平衡因子（BF），等于结点的左子树高度-右子树高度。
平衡二叉树的所有结点的平衡因子只能是-1，0，1，当插入一个新节点后失衡，需要对树进行平衡化
**失衡二叉树的平衡化**

- 调整原则——降低高度、保持二叉树的性质
- <div align="center"> <img src="https://cdn.nlark.com/yuque/0/2022/png/29674612/1661569002224-c09fa08a-0243-4b19-aa44-b047c8dbd17a.png#averageHue=%23faf8f3&clientId=u7ef8cc1b-3114-4&from=ui&height=220&id=ubfe5433a&originHeight=739&originWidth=1692&originalType=binary&ratio=1&rotation=0&showTitle=false&size=436364&status=done&style=none&taskId=ud8b49356-d695-4e7b-9d2a-65e27281807&title=&width=504" width="504" /> </div>
...
- LL型
   ...
   - A节点称为B的右孩子
   - 原来B结点的右子树称为A结点的左子树
   - <div align="center"> <img src="https://cdn.nlark.com/yuque/0/2022/png/29674612/1661569409851-e14b1178-1c60-4ba2-aa94-b84c742642a7.png#averageHue=%23fefefe&from=url&height=222&id=qTxCR&originHeight=448&originWidth=724&originalType=binary&ratio=1&rotation=0&showTitle=false&size=54038&status=done&style=none&title=&width=358" width="358" /> </div>
...
- R型
   ...
   - A节点称为B的左孩子
   - 原来B结点的左子树称为A结点的右子树
   - <div align="center"> <img src="https://cdn.nlark.com/yuque/0/2022/png/29674612/1661569619211-e94b39b4-b3dd-4b88-b148-6673bd45780d.png#averageHue=%23fefefe&from=url&height=219&id=Yz9G3&originHeight=475&originWidth=736&originalType=binary&ratio=1&rotation=0&showTitle=false&size=59052&status=done&style=none&title=&width=339" width="339" /> </div>


- LR型
   - C结点穿过A、B结点上升
   - B称为C的左孩子，A称为C的右孩子
   - 原来C的左子树成为B的右子树，原来C的右子树成为A的左子树
   - <div align="center"> <img src="https://cdn.nlark.com/yuque/0/2022/png/29674612/1661570010707-26bb7329-b75e-4f26-b7cd-2292586f070e.png#averageHue=%23fefefe&from=url&height=223&id=DdISG&originHeight=548&originWidth=631&originalType=binary&ratio=1&rotation=0&showTitle=false&size=71311&status=done&style=none&title=&width=257" width="257" /> </div>


- RL型
   - C结点穿过A、B结点上升
   - A称为C的左孩子，B称为C的右孩子
   - 原来C的左子树成为A的右子树，原来C的右子树成为B的左子树
   - <div align="center"> <img src="https://cdn.nlark.com/yuque/0/2022/png/29674612/1661570172514-b3315b00-b876-4de1-8dab-4243af5f43ad.png#averageHue=%23fefefe&from=url&height=228&id=FzNnn&originHeight=475&originWidth=575&originalType=binary&ratio=1&rotation=0&showTitle=false&size=68058&status=done&style=none&title=&width=276" width="276" /> </div>
...
- 若待插入的结点p已经有$m-1$个关键字，将p分裂长p和$p^{'}$两个结点
   - <div align="center"> <img src="https://cdn.nlark.com/yuque/0/2022/png/29674612/1661684197152-7ac62e38-eeef-437e-b52c-e02702755732.png#averageHue=%23f7f7f7&from=url&height=226&id=C5gYt&originHeight=226&originWidth=1114&originalType=binary&ratio=1&rotation=0&showTitle=false&size=60724&status=done&style=none&title=&width=1114" width="1114" /> </div>
