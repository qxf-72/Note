...
希尔排序的分析相当复杂，但增量序列为 $dlta[k]=2^{k+1}-1$ 时，希尔排序时间复杂度为 $O(n^{3/2})$ 。当 $n$ 位于某个特定范围时，希尔排序时间复杂度为 $O(n^{1.3})$ 。在最坏情况下，希尔排序时间复杂度为 $O(n^2)$ 。
...
计数排序也是一种不基于排序的算法。计数排序思想：对于每一个待排序元素 $x$ ，统计小于 $x$ 的元素个数，利用该
