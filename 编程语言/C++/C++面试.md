...
# Day 3：类、对象、构造析构面试常见问题与答案

  

本文围绕 Day 3 的内容整理：类、对象、构造函数、析构函数、拷贝构造、拷贝赋值、三/五法则、深拷贝和浅拷贝，以及手写 `String` 类。
...
类是用户自定义类型，用来描述一类对象的属性和行为；对象是类的具体实例。
...
> 类是对数据和操作数据函数的封装，对象是类的实例。C++ 通过类实现封装、继承和多态等面向对象特性。
...
在 C++ 中，`class` 和 `struct` 都可以定义类，主要区别是默认访问权限不同。
...
除此之外，`class` 和 `struct` 都可以有构造函数、析构函数、成员函数、继承、多态等。
...
> C++ 中 `class` 和 `struct` 的能力基本一样，区别主要是默认访问权限和默认继承权限不同。`class` 默认 `private`，`struct` 默认 `public`。
...
> `private` 用于隐藏实现细节，`public` 暴露接口，`protected` 常用于继承体系。
...
封装就是把数据和操作数据的函数放在一起，并通过访问权限控制外部访问。
...
这里 `balance` 不允许外部直接修改，只能通过 `deposit()` 这种受控接口操作。
...
> 封装可以隐藏实现细节，保护对象状态，降低模块之间的耦合，提高代码可维护性。
...
`this` 是成员函数内部的隐式指针，指向调用该成员函数的当前对象。
...
> `this` 指针指向当前对象，静态成员函数没有 `this` 指针，因为静态成员函数不属于某个具体对象。
...
构造函数是在对象创建时自动调用的特殊成员函数，用来初始化对象。
...
但是如果你自己定义了有参构造函数，编译器通常不会再自动生成无参默认构造函数。
...
成员初始化列表是在成员真正构造时初始化，构造函数体内赋值是成员已经构造完成之后再赋值。
...
对于普通内置类型，差别不明显；但对于类类型成员，初始化列表通常更高效。
...
如果不用初始化列表，`b` 可能会先默认构造，再在函数体中赋值，效率较低。
...
        x = val;   // 错误，const 成员不能赋值

        ref = r;   // 这不是重新绑定引用，而是给 ref 绑定的对象赋值
...
> 初始化列表是真正的初始化，构造函数体内是赋值。`const` 成员、引用成员、没有默认构造函数的成员对象必须通过初始化列表初始化。
...
成员变量的初始化顺序由它们在类中的声明顺序决定，不由初始化列表中的书写顺序决定。
...
虽然初始化列表中写的是 `y(1), x(y)`，但是实际初始化顺序是：
...
对象在构造过程中，派生类部分还没有构造完成，虚函数机制依赖对象完整的动态类型，而构造函数执行时对象还没有完全形成。
...
> 构造函数不能是虚函数，因为虚函数机制需要依赖对象的动态类型，而构造时对象尚未完整构造。
...
### 14. 析构函数可以是虚函数吗？为什么基类析构函数常常要写成虚函数？
...
如果一个类作为基类，并且可能通过基类指针删除派生类对象，那么基类析构函数应该声明为 `virtual`。
...
delete ptr; // 如果 Base 析构函数不是 virtual，可能不会正确调用 Derived 析构函数
...
> 如果类用于多态，基类析构函数应该是虚函数。否则通过基类指针删除派生类对象时，可能只调用基类析构函数，导致派生类资源泄漏，行为未定义。
...
原因是对象析构常常发生在栈展开过程中，如果析构函数再抛出异常，可能导致程序调用 `std::terminate()` 直接终止。
...
> 析构函数应该尽量保证不抛异常。如果内部操作可能失败，应在析构函数内部捕获并处理异常。
...
> 构造从外到内，析构从内到外。基类先构造后析构，派生类后构造先析构。
...
拷贝构造函数是用一个已有对象初始化一个新对象时调用的构造函数。
...
虽然中间有 `=`，但这是定义新对象时的初始化，因此调用的是拷贝构造函数。
...
2. 使用 `const` 保证不会修改被拷贝对象，并且可以接受 const 对象。
...
调用拷贝构造时需要先拷贝参数 `other`，又会调用拷贝构造，形成无限递归。
...
> 拷贝构造函数参数必须使用引用，否则传参本身就需要调用拷贝构造，会导致无限递归。加 `const` 是为了不修改源对象，并支持拷贝 const 对象。
...
现代 C++ 中，返回值优化和移动语义可能减少或避免拷贝构造调用。
...
如果用户没有定义拷贝构造函数，编译器通常会自动生成一个。

  

自动生成的拷贝构造函数会进行成员逐个拷贝。
...
A b = a; // 编译器生成的拷贝构造可以完成 x 和 y 的拷贝

```

  

但是如果类中管理动态资源，例如裸指针，默认拷贝构造可能产生浅拷贝问题。
...
拷贝赋值运算符用于把一个已有对象的值赋给另一个已经存在的对象。
...
| 发生时机 | 用已有对象初始化新对象 | 两个已存在对象之间赋值 |

| 函数形式 | `A(const A& other)` | `A& operator=(const A& other)` |

| 是否已有旧资源 | 新对象还没有旧资源 | 左侧对象可能已有旧资源 |

| 是否需要释放旧资源 | 通常不需要 | 通常需要先处理旧资源 |
...
> 拷贝构造是“创建新对象”，拷贝赋值是“给已有对象重新赋值”。拷贝赋值通常要注意自赋值和旧资源释放问题。
...
表达式 `b = c` 返回 `b` 的引用，然后再执行 `a = b`。

  

如果返回 `void`，就不能支持这种链式赋值。
...
如果类中管理动态资源，错误的赋值写法可能会先释放自己的资源，然后又从已经释放的资源中拷贝数据。
...
那么 `delete[] data_` 后，`other.data_` 也被释放了，再 `strlen(other.data_)` 就会出问题。
...
浅拷贝是只拷贝对象中的成员值。如果成员是指针，只拷贝指针地址，不拷贝指针指向的资源。
...
1. 一个对象修改数据，另一个对象也受影响；

2. 两个对象析构时会对同一块内存 `delete[]` 两次，导致 double free。
...
深拷贝是不仅拷贝指针成员本身，还会重新申请一份独立资源，并拷贝资源内容。
...
> 浅拷贝只拷贝指针地址，多个对象共享同一份资源，容易导致重复释放。深拷贝会重新分配资源并复制内容，每个对象独立管理自己的资源。
...
当类中直接管理资源时，通常需要自己写拷贝构造、拷贝赋值和析构函数。
...
三法则是指：如果一个类需要自定义以下三个函数中的任意一个，那么通常也需要自定义另外两个。
...
如果类需要自定义析构函数，通常说明它管理资源；既然管理资源，那么默认拷贝构造和默认拷贝赋值往往会产生浅拷贝问题。
...
> 三法则用于管理资源的类。如果需要析构函数释放资源，就通常也需要自己实现拷贝构造和拷贝赋值，避免浅拷贝和重复释放。
...
移动语义的作用是把临时对象或即将销毁对象的资源“转移”过来，避免不必要的深拷贝。
...
> 如果类需要自己管理资源，在 C++11 之后通常要考虑五法则。除了拷贝控制，还要考虑移动构造和移动赋值，以提高性能。
...
零法则是现代 C++ 更推荐的设计方式：尽量不要手动管理资源，而是使用标准库容器或智能指针，让编译器自动生成特殊成员函数。
...
因为 `std::string` 和 `std::vector` 已经正确处理了拷贝、移动和析构。
...
> 现代 C++ 优先遵循零法则，尽量使用 RAII 类型、标准库容器和智能指针管理资源，避免自己写析构、拷贝和移动函数。
...
### 33. 请手写一个简单的 `String` 类，包含构造、析构、拷贝构造和拷贝赋值
...
            char* new_data = new char[std::strlen(other.data_) + 1];
...
### 34. 为什么 `String` 的析构函数中要使用 `delete[]`，不能使用 `delete`？
...
> `new` 对应 `delete`，`new[]` 对应 `delete[]`，必须配对使用。
...
### 35. 为什么 `String(const char* s)` 要多申请一个字符空间？
...
一旦 `new` 失败抛异常，对象原来的数据已经丢失，对象会处于不安全状态。
...
> 先分配新资源成功后，再释放旧资源，可以保证如果分配失败，原对象仍然保持原状态，异常安全性更好。
...
这里参数按值传递，会先生成一份副本；然后当前对象和副本交换资源；函数结束后，副本析构，自动释放旧资源。
...
作用是直接接管 `other` 的资源，避免重新分配和复制字符串内容。

  

注意：移动后要让 `other` 处于可析构的安全状态。
...
String s2 = s1;             // 拷贝构造，复制 hello

String s3 = std::move(s1);  // 移动构造，接管 s1 的资源
...
> 拷贝构造会深拷贝资源，移动构造直接接管临时对象或右值对象的资源，避免额外分配和复制，提高性能。
...
问题是这个类自定义了析构函数，但没有自定义拷贝构造和拷贝赋值。

  

编译器自动生成的拷贝构造会进行浅拷贝，导致：
...
当 `s1` 和 `s2` 析构时，会对同一块内存释放两次，导致未定义行为。

  

解决方法是遵循三法则，实现拷贝构造和拷贝赋值。
...
构造 `B` 对象时，先构造成员对象 `a`，再执行 `B` 自己的构造函数体。

  

析构时顺序相反，先执行 `B` 的析构函数体，再析构成员对象 `a`。
...
通过基类指针删除派生类对象时，如果基类析构函数不是虚函数，派生类析构函数可能不会被正确调用，导致 `Derived` 中的资源 `p` 没有释放。
...
构造函数用于初始化对象，在对象创建时自动调用；析构函数用于清理对象资源，在对象销毁时自动调用。
...
构造函数不能是虚函数，析构函数可以是虚函数，并且多态基类通常应该有虚析构函数。
...
拷贝构造是用已有对象初始化新对象；拷贝赋值是把一个已有对象赋值给另一个已有对象。
...
如果类中管理动态资源，自定义析构函数释放资源后，默认拷贝构造和默认拷贝赋值仍然只是浅拷贝，容易导致多个对象共享同一资源，从而重复释放。
...
C++11 后有移动语义。如果类管理资源，除了处理拷贝，还应该处理移动。
...
1. 忘记给字符串末尾的 `\0` 多申请一个空间；

2. 析构时写成 `delete data_`，而不是 `delete[] data_`；
...
6. `const char*` 构造函数没有处理 `nullptr`；

7. 赋值时先释放旧资源，后申请新资源，异常安全性较差；

8. 移动构造后没有让源对象处于安全可析构状态。
...
> 类是用户自定义类型，对象是类的实例。类通过访问权限实现封装，通过构造函数初始化对象，通过析构函数清理资源。
...
> 构造函数在对象创建时自动调用，用于初始化对象。构造函数可以重载，不能是虚函数。成员初始化列表是真正初始化，构造函数体内是赋值。
...
> 析构函数在对象销毁时自动调用，用于释放资源。析构函数不能重载。作为多态基类时，析构函数应该声明为虚函数。
...
> 构造顺序是：基类、成员对象、派生类自身。析构顺序正好相反。成员变量初始化顺序由声明顺序决定，不由初始化列表顺序决定。
...
> 拷贝构造用于用已有对象初始化新对象，典型形式是 `A(const A& other)`。参数必须用引用，否则会导致无限递归；加 `const` 是为了不修改源对象。
...
> 拷贝赋值用于两个已经存在的对象之间赋值，典型形式是 `A& operator=(const A& other)`。需要注意自赋值、旧资源释放、深拷贝和返回 `*this`。
...
> 浅拷贝只拷贝指针地址，多个对象共享同一资源，容易重复释放。深拷贝会重新申请资源并复制内容，使每个对象独立管理自己的资源。
...
> 如果类需要自定义析构函数、拷贝构造函数、拷贝赋值运算符中的任意一个，通常也需要自定义另外两个。
...
> C++11 后，三法则扩展为五法则，还需要考虑移动构造和移动赋值。移动语义可以转移资源，减少深拷贝，提高性能。
...
> 现代 C++ 更推荐使用标准库容器和智能指针管理资源，让编译器自动生成特殊成员函数，尽量不手写析构、拷贝和移动函数。