...
- 指定生成文件名：`-o` ，例如 `g++  -o   program   program.cpp`
   - 在UNIX系统生成可执行文件 `program.out`
   - 在Windows系统生成 `program.exe`
- 运行可执行文件：
...
命名空间和作用域运算符 `::`：命名空间可以表面变量名字相同引起的冲突，cin、cout和end都属于命名空间std，可以使用作用域运算符指明变量所属的命名空间。
...
<div style="text-align: center;"><img src="https://picture-in-md.oss-cn-guangzhou.aliyuncs.com/2023-01-13_173031.png" loading="lazy" style="max-width: 100%; height: auto; width: 600px;"/> </div>
...
- 带符号类型和无符号类型：
   - 整型（字符除外）都是带符号类型
   - **普通字符型分为三种：char、signed char、unsigned char**，其中表现形式只有两种：带符号和无符号，类型char会表现为两种标准形式其中一种，具体由编译器决定。

> 虽然 char 可能表现得像 signed char，但它并不等于 signed char。
> 
> 例如：
> 
> ```cpp
...
<div style="text-align: center;"><img src="https://picture-in-md.oss-cn-guangzhou.aliyuncs.com/2023-01-15_092910.png" loading="lazy" style="max-width: 100%; height: auto; width: 600px;"/> </div>


> 注意以上的前缀部分，中文字符串是以字符数组方式存储的，所以在给字符数组赋值时需要在字符串常量前面加上修饰的前缀。

> ```cpp
> char s1[] = "你好";        // 普通窄字符串，通常按 UTF-8 或本地编码存储
> char16_t s2[] = u"你好";   // UTF-16
> char32_t s3[] = U"你好";   // UTF-32
> wchar_t s4[] = L"你好";    // 宽字符字符串
> ```


> 全部采用 utf 8 存储、表示，就不会出现乱码。使用 `chcp` 查看当前终端的默认代码页，如果不是 `65001`，就 `chcp 65001`。
...
- 定义：提供一个具有名字的、可供程序操作的存储空间 （涉及**内存操作** ）。
...
- 声明：使得名字被程序所知，如果要声明一个变量而不定义，在变量名前加上关键字 `extern` ，另外，**任何有显式初始化的声明就成了定义**。
- 定义：创建与名字关联的实体，即申请内存。
...
- 空指针：C++11引入一个新的字面值  `nullptr`  表示空指针，新标准下避免使用 `NULL` （预处理变量，值为 0 ）。
- `void*` 指针：一种特殊指针类型，可以存放任意对象地址，不能直接操作 void 所指对象，因为并不清楚对象是何类型。
...
**也就是说不同文件，如果最后需要链接在一起，其中不能有相同的普通全局变量，但是可以有相同 const 变量。**
...
### 2.4.2  `const` 的引用
可以把引用绑定到 `const` 对象上，称为对常量的引用；一般而言，引用类型必须与其所引用的对象类型一致，但也有例外的情况：

- 初始化 **const引用**允许用任意表达式作为初始值，只要表达式结果能够转化为引用的类型
...
- **底层 `const` **：**指针所指对象是一个常量**，同样，底层const也能推广到引用。
...
> - 没有定义构造函数时，只有真正创建对象时，才会调用默认构造函数
> - **内置类型的默认初始化就是不初始化**。

> <div style="text-align: center;"><img src="https://picture-in-md.oss-cn-guangzhou.aliyuncs.com/2026-05-25_10-10-08.png" loading="lazy" style="max-width: 100%; height: auto; width: 400px;"/> </div>
...
> <div style="text-align: center;"><img src="https://picture-in-md.oss-cn-guangzhou.aliyuncs.com/2026-05-25_10-13-45.png" loading="lazy" style="max-width: 100%; height: auto; width: 600px;"/> </div>
...
- 直接从输入流输入 `>>string` 中，==string 只读取到单个词（跳过前面空白，后面遇到空白就停止）==。
...
- 在 C++ 中，左值表达式表示一个对象或函数的身份。左值不一定可修改，例如 `const` 对象也是左值，但不能作为赋值运算符左侧的可修改运算对象。  
- 一个表达式即使求值结果是对象，也不一定是左值，例如字面值、算术表达式结果、函数按值返回的结果通常是右值。  
- 当对象被用作右值时，使用的是对象的值；当对象被用作左值时，使用的是对象的身份，也就是它在程序中的位置或实体本身。
...
> C++ 中自动类型转换的目的是在表达式求值、赋值、初始化或函数调用时，将操作数转换为目标类型或共同类型，使运算或调用能够按照统一规则进行。对于算术运算，一般会先整型提升，再进行通常算术转换。但不能简单理解为永远向更大的类型转换，尤其是有符号和无符号混用时，可能会把负数转换成很大的无符号数，导致结果反直觉。
...
> <div style="text-align: center;"><img src="https://picture-in-md.oss-cn-guangzhou.aliyuncs.com/2026-05-25_10-40-29.png" loading="lazy" style="max-width: 100%; height: auto; width: 700px;"/> </div>
...
- 标签标示符独立于变量或其他标识符的名字，可以和程序中其他实体标识符同名。
- goto语句和控制权转向的标签必须位于同一个函数内。
	- 跨函数跳转会绕过函数调用机制，导致目标函数的栈帧、参数、局部变量、对象生命周期和返回地址都不正确。因此 C++ 规定 `goto` 和它跳转的标签必须在同一个函数内。
- 向后跳过一个已经执行的定义，意味着系统将销毁该变量然后重新创建。
	- `goto` 从变量定义之后，跳回到变量定义之前，那么这次跳转会离开该变量的生命周期。  
	- 对于类类型对象，离开生命周期时会调用析构函数；然后程序再次执行到定义语句时，会重新调用构造函数。
...
- **可变数据成员**：有时需要修改一个数据成员的值，即使是在const成员函数里面，可以在该**变量前面加上`mutable`关键字，表示可以被const成员函数修改**。
...
> const成员函数和普通成员函数的区别：
> - const成员函数是指在函数声明或定义时在函数名后加上const关键字，表示该函数不能修改类的成员变量（mutable、static 除外），**也不能调用非const的成员函数**。
...
- 对于不完全类型，可以**定义**指向不完全类型的指针和引用，**声明**以不完全类型作为参数或返回值的函数（当然**无法定义**），所以**一个类中的成员不能是该类自己，但是可以是指向他自己的指针或引用**。
...
在C++中，用`new`分配程序堆空间，用`delete`释放申请的内存。为了安全使用动态内存，新标准库提供了两种智能指针——`shared_ptr`和`unique_ptr`，可自动释放指向的对象。还定义了一种弱引用`weak_ptr`，==三者定义在头文件`memory`中。==
...
**成员函数与继承**
派生类可以继承基类的成员，==派生类可以提供自己的定义来覆盖继承而来的定义（**是覆盖定义，继承而来的成员还是存在的**）==。
